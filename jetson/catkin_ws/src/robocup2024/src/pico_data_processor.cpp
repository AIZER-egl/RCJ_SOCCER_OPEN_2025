...
unsigned long long previous_kick_time = 0;
unsigned long long previous_time_front_camera_blockage = 0; 
...
void controlLoopCallback(const ros::TimerEvent& event) {
    ...
    if (kick_requested && (current_time - previous_kick_time > 10000)) {
        data.kicker_active = true;
        previous_kick_time = current_time;
        ROS_INFO("Kicker activated!");
    } else {
        data.kicker_active = false;
    }
    
    if (front_angle == INVALID_VALUE && !first_front_camera_detection) {
         first_front_camera_detection = true;
    }

    if (current_time - previous_time_front_camera_blockage <= 800) {
    ...
    if (front_angle != INVALID_VALUE && !data.robot_stop) {
            data.robot_direction = static_cast<int16_t>(front_angle * 1.3);
            data.robot_speed = 50;
            data.robot_facing = 0;
            data.robot_stop = false;
    } else if (omni_angle != INVALID_VALUE && !data.robot_stop) {
            data.robot_direction = static_cast<int16_t>(omni_angle + (45.0 * signOf(static_cast<float>(data.robot_direction))));
            data.robot_speed = 60;
            data.robot_facing = 0;
            data.robot_stop = false;
    } else if (!data.robot_stop) {
            data.robot_direction = 0;
            data.robot_speed = 0;
            data.robot_facing = 0;
            data.robot_stop = false;
    ...
    if (!bytes_to_send.empty()) {
        ...
        ROS_INFO("Message published to serial_tx (Dir=%d, Speed=%d, Kicker=%d, Stop=%d, Yaw: %d, LDR: %d)",
                 data.robot_direction, data.robot_speed, data.kicker_active, data.robot_stop, data.compass_yaw, data.ldr_value);
    } else {
...
int main (int argc, char **argv) {
...

    {
        std::lock_guard<std::mutex> lock(g_data_mutex);
        data = {};
        previous_time_front_camera_blockage = millis();
        g_new_pico_data_received = false;
    }
